// Main program for s5933dk1 driver
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT pdo)
	...
	RtlInitUnicodeString(&devname, L"\\DosDevices\\S5933DK1");
	...
	do
		...
		// Initialize the work item plumbing

		KeInitializeEvent(&pdx->evKill, NotificationEvent, FALSE);
		KeInitializeEvent(&pdx->evWork, NotificationEvent, FALSE);
		KeInitializeSpinLock(&pdx->QueueLock);
		ExInitializeFastMutex(&pdx->QueueMutex);
		InitializeListHead(&pdx->WorkItems);
