// Main program for devprop driver
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT pdo)
	...
	_snwprintf(namebuf, arraysize(namebuf), L"\\Device\\DEVPROP_%d", InterlockedIncrement(&devcount));
	...
	do
		...
		// Print a debugging message that includes the device description from the
		// registry.
		...
		// Print the FriendlyName, if there is one. In this case, we need a runtime check for
		// the OS platform because Win98 implements this one property incorrectly (it gets the
		// friendly name from the software key instead of the hardware key)
		...
		if (win98)
			...
			if (NT_SUCCESS(status))
				...
				if (status != STATUS_OBJECT_NAME_NOT_FOUND && size && size < PAGE_SIZE)
					{					// value exists
					...
					if (NT_SUCCESS(status))
						KdPrint((DRIVERNAME " - Friendly name of device is '%ws' device\n", vpip->Data));
					ExFreePool(vpip);
					}				// value exists
				ZwClose(hkey);
				}					// get & report description
	...
	#endif // _X86_
			{						// get property
			status = IoGetDeviceProperty(pdo, DevicePropertyFriendlyName, sizeof(name), name, &junk);
			if (NT_SUCCESS(status))
				KdPrint((DRIVERNAME " - Friendly name of device is '%ws' device\n", name));
			else
				KdPrint((DRIVERNAME " - IoGetDeviceProperty failed - %X\n", status));
